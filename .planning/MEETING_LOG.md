...
## 회의 2026-04-28-γ — 결제 채널 이중 구조 카피·약관·정책 전면 정합성 정리

**발단:** Paddle Live 전환(2026-04-28-α) 완료 후 대표가 "결제 FAQ KO/EN 양쪽에 카카오페이뿐 아니라 Paddle 추가 + CURRENT_STATE/정책/이용약관/랜딩 등 수정 필요한 곳 전부 정리, 기획자/평가자 교차 검증으로 빠짐 없이"를 지시.

### 기획자/평가자 교차 검증 발견 (3 Critical / 4 Warning)

**🔴 Critical:**
1. EN 약관(`/en/terms.tsx:42`) "billed monthly via KakaoPay" — Paddle 결제자 법적 근거 부재
2. EN FAQ(`en.json:701-703`) "Currently only KakaoPay is available" — Paddle Live 후 거짓 정보
3. KO/EN 개인정보 처방침에 Paddle.com Market Ltd. 위탁사 누락 — GDPR/개인정보보호법 미준수 위험

**🟡 Warning:**
4. EN 환불정책(`/en/refund.tsx:32`) Paddle 처리 시간 미정의
5. KO 환불정책(`refund.tsx:43`) "카카오페이 기준 3~5 영업일"만 명시
6. SubscriptionScreen 내장 TERMS_TEXT/REFUND_EN 동일 옛 표기
7. 랜딩 FAQ(`landingTexts.ts:166-167, 276-277`) 결제 수단 명시 없음

### UnivAI 환불 정책 비교 검토 (대표 제시)
- UnivAI는 Paddle 미사용 (PortOne + Apple In-App). 일수 명시 없음.
- 우리는 Paddle 이중 채널이라 일수 명시 필수. **5~10 영업일** 표기는 Paddle 공식 가이드("typically 5-10 business days") 그대로.
- UnivAI 정책에서 차용한 보강 1건: **회사 귀책 예외 조항** (시스템 오류·중복 결제 시 기한·이용 이력 제한 없이 환불) — 우리 정책 신설.

### 결정 — 8개 영역 16곳 패치 (6 분리 커밋)

| 영역 | 파일 |
|---|---|
| 1. i18n FAQ | `ko.json:697-699` + `en.json:701-703` |
| 2. 약관 (terms) | `terms.tsx:97-99` + `en/terms.tsx:42, 48` |
| 3. 환불정책 | `refund.tsx:43+` + `en/refund.tsx:32+` |
| 4. 개인정보 처방침 | `privacy.tsx:91` + `en/privacy.tsx:45` |
| 5. SubscriptionScreen 내장 | `SubscriptionScreen.tsx:35, 155, 292, 364, 370+` |
| 6. 랜딩 FAQ | `landingTexts.ts:166-167, 276-277` |
| 7. CURRENT_STATE.md | SubscriptionScreen 섹션 갱신 |
| 8. CLAUDE.md | 환경변수 섹션 + functions secrets 명시 |

### 카피 통일 결정
- **환불 처리 시간:** "카카오페이 3~5 영업일 / Paddle 5~10 영업일" — Paddle 공식 가이드 그대로
- **면책 한 줄:** "(카드사 정책에 따라 변동 가능)" / "(some banks may take longer)"
- **회사 귀책 예외 조항:** 약관·환불정책·SubscriptionScreen 모두 신설
- **위탁사 표기:** "Paddle.com Market Ltd. (Merchant of Record)" 풀네임
- **결제 수단 분기 안내:** "한국 사용자는 카카오페이(PortOne), 해외 사용자는 신용카드(Paddle)"

### 평가자 자가 비판
- 1차 평가자 보고에서 HMAC 검증 + customData 필드명 2건을 🔴 Critical 로 잘못 단정 ([feedback_evaluator_strict] 룰 일부 위반). orchestrator 직접 검증으로 정정. 단 Paddle 환불 API 누락 1건 발굴은 정확 — 회의 2026-04-28-α에서 패치 완료.
- 2차 평가자 보고는 라인 번호 + before/after 형식으로 정확. orchestrator 직접 검증 통과.

### 커밋 계획
1. `chore(i18n): FAQ 결제 수단 Paddle 추가 (ko/en)`
2. `chore(legal): 약관·환불정책·개인정보 처방침 Paddle provider 명시 (KO/EN 6 파일) + 회사 귀책 예외 조항 신설`
3. `chore(subscription): SubscriptionScreen 내장 카피 Paddle 동기화`
4. `chore(landing): 랜딩 FAQ 결제 수단 Paddle 반영 (ko/en)`
5. `docs(current-state): Paddle Live 전환 + 환불 정책 분기 반영 + MEETING_LOG 갱신`
