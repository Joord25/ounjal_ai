...
export const FitScreen: React.FC<FitScreenProps> = ({
  ...
  const playAlarmSound = (pattern: "start" | "tick" | "half" | "end" | "rest_end" | "exercise_done" = "end") => {
    try {
      const ctx = getAudioCtx();
      const t = ctx.currentTime;

      // 마스터 컴프레서 — 큰 볼륨 + 클리핑 방지
      ...
      const ripple = (start: number, vol: number) => {
        const notes = [1047, 1319, 1568, 1760, 2093]; // C6-C7 상승 펜타토닉
