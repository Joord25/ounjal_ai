        ...
        <div className="mt-3 mb-1">
          {monthHistory.length > 0 ? (
            <div className="flex items-center gap-4 px-2">
              ...
              <p className="text-xs text-[#95D5B2]/50 ml-auto">
                <span className="text-sm font-black text-white/90">{Math.round(monthHistory.reduce((s, h) => s + (h.stats.totalVolume || 0), 0)).toLocaleString()}</span> kg · <span className="text-sm font-black text-white/90">{Math.round(monthHistory.reduce((s, h) => s + (h.stats.totalDurationSec || 0), 0) / 60)}</span> {locale === "ko" ? "분" : "min"} · <span className="text-sm font-black text-white/90">{monthHistory.reduce((s, h) => s + (h.stats.totalSets || 0), 0)}</span> {locale === "ko" ? "세트" : "sets"}
              </p>
            </div>
          ) : isCurrentMonth ? (
