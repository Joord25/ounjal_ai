...
export const LABELED_EXERCISE_POOLS: { label: string; keywords: string[]; exercises: string[] }[] = [
  ...
  { label: "삼두", keywords: ["삼두", "트라이셉", "tricep", "팔뒤"], exercises: ["트라이셉 로프 푸쉬다운 (Tricep Rope Pushdown)", "스컬 크러셔 (Skullcrushers)", "오버헤드 트라이셉 익스텐션 (Overhead Tricep Extension)", "케이블 푸쉬 다운 (Cable Pushdown)", "케이블 오버헤드 트라이셉 익스텐션 (Cable Overhead Tricep Extension)", "트라이셉스 킥백 (Tricep Kickback)", "트라이셉스 딥스 (Tricep Dips)", "클로즈그립 벤치 프레스 (Close-Grip Bench Press)"] },
  { label: "후면 어깨", keywords: ["후면", "rear", "리어"], exercises: ["케이블 페이스 풀 (Cable Face Pulls)", "리어 델트 플라이 (Rear Delt Fly)", "밴드 풀 어파트 (Band Pull-Aparts)", "리버스 스노우 엔젤 (Reverse Snow Angel)", "Y-T-W 레이즈 (Y-T-W Raises)"] },
